...
/// Validate the ed25519 instruction, and extract `pubkey` and `message`
...
pub fn ed25519_verify(
    ...
    // `new_ed25519_instruction` generates an instruction that must be at index 0.
    let ed25519_instr =
        sysvar::instructions::load_instruction_at_checked(0, instructions_sysvar_account)?;

    // Verify it is a ed25519 instruction.
    // This is security-critical - what if the transaction uses an imposter ed25519 program?
    ...
    for offsets in ed25519_defs::iter_signature_offsets(&ed25519_instr.data)? {
        // `new_ed25519_instruction` generates an instruction that only uses instruction index 0.
...
mod ed25519_defs {
    ...
    /// The structure encoded in the Ed25519 instruction data.
